...
![os-active Windows 主界面](assets/screen/windows.png)

> 主界面:大字状态英雄区(已激活/未激活) + 底部彩色确认按钮(颜色随状态:绿=已激活/红=未激活/蓝=无需激活/灰=检测中) + 系统信息 + 检测明细表格 + 日志区;确认后倒计时自动关闭,退出输出 ETest `R<...>R` 结果。
